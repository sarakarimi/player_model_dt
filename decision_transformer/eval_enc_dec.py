...
from util import get_max_len_from_model_type, initialize_padding_inputs
...
def evaluate_dt_agent(
    ...
    if not hasattr(model, "transformer_config"):
        model.transformer_config = Namespace(
            n_ctx=model.dt.n_ctx,
            time_embedding_type=model.dt.time_embedding_type,
        )
    ...
    obs, actions, reward, rtg, timesteps, mask, _ = initialize_padding_inputs(
        max_len=max_len,
        initial_obs=obs,
    ...
    full_timesteps = full_timesteps.to(device)
    ...
    output = model.forward(
        ...
        full_attn_masks=full_attention_mask,
    )
    ...
    while n_terminated + n_truncated < trajectories:
        ...
        if sum(dones) > 0:
            ...
            # Note: encoder inputs (full_states, etc.) remain the same across resets
            # since they define the style for all episodes in this evaluation
        ...
        if np.any(dones):
            ...
            if track and (
                ...
                for new_video in new_videos:
                    ...
                    wandb.log(
                        {
                            f"media/video/{initial_rtg}/mode_{style_id}/": wandb.Video(
                                path_to_video,
                                fps=4,
                                format="mp4",
                                caption=f" after {n_terminated + n_truncated} episodes, reward {new_reward}, rtg {initial_rtg}",
                            )
                        },
                        step=batch_number,
                    )
    ...
    if track:
        ...
        for key, value in statistics.items():
            ...
            if key == "traj_lengths":
                wandb.log(
                    {
                        f"eval/{str(initial_rtg)}/mode_{style_id}/traj_lengths": wandb.Histogram(
                            value
                        )
                    },
                    step=batch_number,
                ...
                wandb.log(
                    {
                        f"eval/{str(initial_rtg)}/mode_{style_id}/rewards": wandb.Histogram(
                            value
                        )
                    },
                    step=batch_number,
            ...
            wandb.log(
                {f"eval/{str(initial_rtg)}/mode_{style_id}/" + key: value}, step=batch_number
            )
