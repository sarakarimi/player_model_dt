...
def evaluate_dt_agent(
    ...
    obs, actions, reward, rtg, timesteps, mask, prompts = initialize_padding_inputs(
        max_len=max_len,
        initial_obs=obs,
    ...
    _, action_preds, _ = model.forward(
        states=obs, actions=actions, rtgs=rtg, timesteps=timesteps, prompt=prompts,
    )
    ...
    while n_terminated + n_truncated < trajectories:
        ...
        if sum(dones) > 0:
            ...
            if soft_prompt_mode:
                prompts[batch_reset_indexes] = _prompt.to(dtype=prompts.dtype).clone()
