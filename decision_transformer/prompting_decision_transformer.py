...
from configs import EnvironmentConfig, TransformerModelConfig
...
class TrajectoryTransformer(nn.Module):
    ...
    def initialize_easy_transformer(self):
        ...
        cfg = HookedTransformerConfig(
            ...
            n_ctx=self.transformer_config.n_ctx + (self.transformer_config.prompt_len * 3), # accounts for the prompt tokens
...
class PromptingDecisionTransformer(TrajectoryTransformer):
    def __init__(self, environment_config, transformer_config, device, **kwargs):
        ...
        self.prompt_time_embedding = self.initialize_prompt_time_embedding()
        self.prompt_state_embedding = self.initialize_prompt_state_embedding()
        self.prompt_action_embedding = self.initialize_prompt_action_embedding()
        self.prompt_return_embedding = torch.nn.Linear(1, self.transformer_config.d_model, bias=False)
    ...
    def initialize_prompt_action_embedding(self):
        prompt_action_embedding = nn.Sequential(
            nn.Embedding(
                self.environment_config.action_space.n + 1,
                self.transformer_config.d_model,
            )
        )
        # Initialize weights
        nn.init.normal_(prompt_action_embedding[0].weight,
            mean=0.0,
            std=1
                / (
                        (self.environment_config.action_space.n + 1 + 1)
                        * self.transformer_config.d_model
                ),
        )

        return prompt_action_embedding

    def initialize_prompt_time_embedding(self):
        if not (self.transformer_config.time_embedding_type == "linear"):
            self.prompt_time_embedding = nn.Embedding(
                self.environment_config.max_steps + 1,
                self.transformer_config.d_model,
            )
        else:
            self.prompt_time_embedding = nn.Linear(1, self.transformer_config.d_model)

        return self.prompt_time_embedding

    def initialize_prompt_state_embedding(self):
        # if self.transformer_config.state_embedding_type.lower() == "cnn":
        #     state_embedding = MiniGridConvEmbedder(
        #         self.transformer_config.d_model, endpool=True
        #     )
        ...
        prompt_state_embedding = nn.Linear(
            n_obs, self.transformer_config.d_model, bias=False
        )

        nn.init.normal_(prompt_state_embedding.weight, mean=0.0, std=0.02)

        return prompt_state_embedding

    def get_prompt_state_embedding(self, states):
        # embed states and recast back to (batch, block_size, n_embd)
        block_size = states.shape[1]
        if self.transformer_config.state_embedding_type.lower() in [
            "cnn",
    ...
    def get_prompt_action_embedding(self, actions):
        block_size = actions.shape[1]
        if block_size == 0:
            return None  # no actions to embed
        actions = rearrange(
    ...
    def get_prompt_time_embedding(self, timesteps):
        assert (
                timesteps.max() <= self.environment_config.max_steps
        ), "timesteps must be less than max_timesteps"
    ...
    def get_prompt_return_embedding(self, rtgs):
        block_size = rtgs.shape[1]
        rtgs = rearrange(rtgs, "batch block rtg -> (batch block) rtg")
        rtg_embeddings = self.prompt_return_embedding(rtgs.type(torch.float32))
    ...
    def to_tokens(self, states, actions, rtgs, timesteps, prompt=None):
        ...
        if prompt is not None:
            ...
            prompt_states, prompt_actions, prompt_returns_to_go, prompt_timesteps = prompt
            prompt_seq_length = prompt_states.shape[1]
            prompt_state_embeddings = self.get_prompt_state_embedding(prompt_states)
            prompt_action_embeddings = self.get_prompt_action_embedding(prompt_actions)
            if prompt_returns_to_go.shape[1] % 10 == 1:
                prompt_returns_embeddings = self.get_prompt_return_embedding(prompt_returns_to_go[:, :-1])
            else:
                prompt_returns_embeddings = self.get_prompt_return_embedding(prompt_returns_to_go)
            prompt_time_embeddings = self.get_prompt_time_embedding(prompt_timesteps)
            ...
            ).permute(0, 2, 1, 3).reshape(prompt_states.shape[0], 3 * prompt_seq_length, self.transformer_config.d_model)

            # stacked_inputs add prompted sequence
            if prompt_stacked_inputs.shape[1] == 3 * seq_length:  # if only smaple one prompt
                prompt_stacked_inputs = prompt_stacked_inputs.reshape(1, -1, self.transformer_config.d_model)
                token_embeddings = torch.cat((prompt_stacked_inputs.repeat(batch_size, 1, 1), token_embeddings), dim=1)

            else:  # if sample one prompt for each traj in batch
                token_embeddings = torch.cat((prompt_stacked_inputs, token_embeddings), dim=1)
        return token_embeddings
    ...
    def get_logits(self, x, batch_size, seq_length, no_actions: bool, prompt=None):
        if no_actions is False:
            ...
            if prompt is None:
                ...
                # x = x[:, 1:, :] # remove style_vector or prompt token
                x = x.reshape(
            ...
            action_preds =  self.predict_actions(x[:, 1])[:, -seq_length:, :] # self.predict_actions(x[:, 1, :, :]) #
