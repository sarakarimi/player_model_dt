...
class TrajectoryTransformer(nn.Module):
    ...
    def initialize_easy_transformer(self):
        ...
        cfg = HookedTransformerConfig(
            ...
            n_ctx=self.transformer_config.n_ctx + 1 +1, # 1 accounts for the style token / prompt token
...
class StyleDecisionTransformer(TrajectoryTransformer):
    def __init__(self, environment_config, transformer_config, device, **kwargs):
        ...
        self.style_embedding = nn.Linear(environment_config.style_vector_size, self.transformer_config.d_model, bias=False)
    ...
    def to_tokens(self, states, actions, rtgs, timesteps, prompt=None):
        ...
        if prompt is not None:
            ...
            style_embeddings = self.style_embedding(prompt.type(torch.float32))
            style_stacked_inputs = style_embeddings
            assert style_stacked_inputs.shape == (batch_size, 1, self.transformer_config.d_model)
