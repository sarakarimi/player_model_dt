...
class MiniGridDungeonEnv(gym.Env):
    """
    ...
    def __init__(
        ...
        self.use_reward_enemy = reward_enemy
        self.reward_treasure = 0.0
        ...
        # Current position, direction and state of the agent
        self.agent_pos: np.ndarray | tuple[int, int] = None
        self.agent_dir: int = None
        self.agent_state: int = None
    ...
    def hash(self, size=16):
        ...
        to_encode = [self.grid.encode().tolist(), self.agent_pos, self.agent_dir, self.agent_state]
        ...
        for j in range(self.grid.height):
            for i in range(self.grid.width):
                if i == self.agent_pos[0] and j == self.agent_pos[1]:
                    ...
                    output += 2 * AGENT_STATE_TO_STR[self.agent_state]
                    continue
    ...
    def adjacent_pos(self):
        """
        Get the position of the cells that are adjacent to the agent
        """
        return [self.agent_pos + DIR_TO_VEC[i] for i in range(4)]
    
    def adjacent_minus_front_pos(self):
        """
        Get the position of the cells that are adjacent to the agent, but not in front of him
        """
        return [self.agent_pos + DIR_TO_VEC[i] for i in range(4) if i != self.agent_dir]
    ...
    def step(
        ...
        truncated = False
        reward = 0.0
        print(f"agent_state: {self.agent_state}")
        ...
        # Move forward
            ...
            if fwd_cell is not None and fwd_cell.type == "goal":
                ...
                if self.use_reward_treasure:
                    reward *= self.reward_treasure
                if self.use_reward_enemy:
        ...
        # Pick up an object
            ...
            if fwd_cell and fwd_cell.can_pickup():
                if fwd_cell.type == "weapon":
                    self.agent_state = 1
                    fwd_cell.cur_pos = np.array([-1, -1])
                    self.grid.set(fwd_pos[0], fwd_pos[1], None)
                if fwd_cell.type == "treasure" and self.use_reward_treasure:
        ...
        # Check if there is an enemy in the adjacent cells minus the front - if there is, the agent loses.
    ...
    def gen_obs(self):
        ...
        obs = {"image": image, "direction": self.agent_dir, "state": self.agent_state, "mission": self.mission}
