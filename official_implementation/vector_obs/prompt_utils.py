...
from evaluation.prompt_evaluate_episodes import prompt_evaluate_episode_rtg
from official_implementation.vector_obs.util_functions import make_dataset
...
def get_prompt(prompt_trajectories, info, variant):
    ...
    def fn(sample_size=1):
        ...
        for i in range(int(num_episodes * sample_size)):
            ...
            si = max(0, traj['reward'].shape[0] - max_len - 1)  # select the last traj with length max_len
    ...
    def fn(batch_size=per_env_batch_size):
        ...
        for env_id, env_name in enumerate(train_env_name_list):
            if prompt_trajectories_list:
                get_prompt_fn = get_prompt(prompt_trajectories_list[env_id], info[str(env_name)], variant)
            else:
                get_prompt_fn = get_prompt(trajectories_list[env_id], info[str(env_name)], variant)
            get_batch_fn = get_batch(trajectories_list[env_id], info[str(env_name)], variant)
        ...
        p_s, p_a, p_r, p_d = torch.cat(p_s_list, dim=0), torch.cat(p_a_list, dim=0), torch.cat(p_r_list,
                                                                                               dim=0), torch.cat(
            p_d_list, dim=0)
        p_rtg, p_timesteps, p_mask = torch.cat(p_rtg_list, dim=0), torch.cat(p_timesteps_list, dim=0), torch.cat(
            p_mask_list, dim=0)
...
def make_prompt_dataset(prompt_save_path):
    pass


def load_data_prompt(env_name_list, data_save_path, prompt_save_path):
    trajectories_list = []
    ...
    for env_name in env_name_list:
        ...
        dataset_path = f"/home/sara/repositories/player_model_dt/data/official_implementation_datasets/restricted_double_goal_mode_{env_name}_image_ft_0_nsteps_100000_ppo_num_episodes100000_eps_0_gamma_0.99_dr_False.npz"
        # prompt_path = f"/home/sara/repositories/player_model_dt/data/official_implementation_datasets/restricted_double_goal_mode_{env_name}_image_ft_0_nsteps_100000_ppo_num_episodes100000_eps_0_gamma_0.99_dr_False_prompt.npz"

        trajectories = make_dataset(dataset_path)
        # prompt_trajectories = make_prompt_dataset(prompt_path)
        trajectories_list.append(trajectories)
        # prompt_trajectories_list.append(prompt_trajectories)
    print(len(trajectories_list[0]))
